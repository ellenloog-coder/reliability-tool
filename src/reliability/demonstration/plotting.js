...
export function demonstrationEvidenceChartSvg(result, labels = {}) {
  ...
  return `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="${escapeHtml(labels.evidenceChart || "Demonstration Evidence")}">
    ...
    ${gapMarker}
    <text x="${width / 2}" y="${height - 14}" text-anchor="middle">${escapeHtml(labels.x || (sample ? "Sample Size" : "Total Test Time"))}</text>
    <text transform="translate(18 ${height / 2}) rotate(-90)" text-anchor="middle">${escapeHtml(labels.achievedConfidence || "Achieved Confidence")}</text>
    <text x="${width - m.r - 4}" y="${(sy(requiredConfidence) - 7).toFixed(1)}" text-anchor="end" class="label">${escapeHtml(labels.requiredConfidence || "Required Confidence")} ${formatPercent(requiredConfidence)}</text>
  </svg>`;
}
